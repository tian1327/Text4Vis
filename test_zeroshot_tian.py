...
def main(args):
    ...
    if os.path.isfile(class_feats_file):
        ...
        with torch.no_grad():
            ...
            print('=> saved classes features to {}'.format(class_feats_file))            
        # if dist.get_rank() == 0:
        #     torch.save(classes_features.cpu(), class_feats_file)
        #     print('=> save classes features to {}'.format(class_feats_file))

    print('type(classes_features): {}'.format(type(classes_features)))
    print('classes_features.shape: {}'.format(classes_features.shape))
